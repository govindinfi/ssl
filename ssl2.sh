...
openssl req -new -config $CONFIG -key server.key -subj "/C=IN/ST=Noida/L=Noida/O=Infinitylabs/OU=Automation/CN=$HOST_IP/emailAddress=govind.kumar@infinitylabs.in" -out server.csr &>/dev/null
...
rm -rf ca.db.*
rm -rf ca.key
echo Installing apache/httpd certificates........
